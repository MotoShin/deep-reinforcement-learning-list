...
from a2c_lstm.agent import Agent, MasterAgent, TestAgent
from a2c_lstm.settings import *
...
class Simulation(object):
    ...
    def one_simulation_start(self, simulation_num):
        ...
        for n in range(UPDATE_NUM):
            ...
            list_s2 = [trajectory["s2"][-1] for trajectory in trajectories]
            values, _ = self.master_agent.get_netowrk_outputs(torch.from_numpy(np.array(list_s2)).type(DTYPE) / 255.0)
    ...
    def test_play(self):
        ...
        agent = TestAgent(self.env.get_n_actions())
